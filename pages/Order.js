  ...
  const executeDelete = async () => {
    ...
    try {
      const response = await fetch(
        `${NGROK_URL}/product?email=${userEmail}&productId=${pendingDeleteId}`,
        { method: "DELETE" }
      );
      const data = await response.json();
...
const styles = StyleSheet.create({
  ...
  header: { paddingTop: 60, paddingBottom: 20, paddingHorizontal: 20, backgroundColor: "#f9f9f9", borderBottomWidth: 1, borderBottomColor: "#eee" },
  ...
  sweetBtn: { flex: 1, paddingVertical: 12, borderRadius: 10, alignItems: 'center', backgroundColor: 'purple' },
